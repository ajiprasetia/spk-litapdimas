// src/components/Admin/ManajemenUser/PengajuanPeneliti.jsx
...
const PengajuanPeneliti = () => {
  ...
  const loadPengajuan = async () => {
    try {
      setLoading(true);
      const data = await PenelitiServices.getAllPengajuan();
      setPengajuan(data);
    } catch (error) {
  ...
  const handleVerifikasi = async (e) => {
    ...
    try {
      const response = await PenelitiServices.verifikasiPengajuan(
  ...
  const filteredPengajuan = pengajuan.filter((item) => {
    ...
    const matchStatus =
      statusFilter === "" || item.status_peneliti === statusFilter;
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Kelola dan verifikasi pengajuan pendaftaran peneliti
          </p>
        </div>
      </div>
                ...
                <tr key={item.id}className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      {getStatusIcon(item.status_peneliti)}
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-semibold ${getStatusColor(
                          item.status_peneliti
                        )}`}
                      >
                        {item.status_peneliti}
                      </span>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-xm font-medium">
                    {item.status_peneliti === "Menunggu Persetujuan" && (
                      <button
                    ...
                    {item.status_peneliti === "Revisi Pengajuan" && (
                      <div className="text-sm">
                        -
                      </div>
                    )}
                    {item.status_peneliti === "Terdaftar" && (
                      <div className="text-sm">
                        -
                      </div>
                    )}
